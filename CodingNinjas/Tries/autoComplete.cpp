...
void insert(string st,trieNode *head)
{
	trieNode *root = head;
...
void giveSuggestion(trieNode *root)
{
	trieNode *head = root;
	if(root->isWord == true)
	{
	...
	for(int i=0;i<26;i++)
	{
		...
		{
			giveSuggestion(root->children[i]);
		}
	}
	
}

void getSuggestion(string s,trieNode *head)
{
	trieNode *root = head;
